    ...
    return (
        <div className={`flex flex-col gap-6 py-16 px-[5%] lg:px-[10%] xl:px-[15%] ${imageAlignment=== 'left' ? 'md:flex-row bg-gradient-to-r from-red-950 via-black to-black' : 'md:flex-row-reverse  bg-gradient-to-l from-red-950 via-black to-black'} `} >
            <div className='flex gap-4 w-full md:w-1/2 lg:w-1/3'>
                <div >
                    {isMedia(firstImage) && (
                        <Image src={awsUrl + firstImage.filename} alt={heading} width={600} height={800} className=' -translate-y-5 rounded-md transform transition-transform duration-300 hover:scale-105 border-4 border-white' />
                    )}
                </div>
                <div>
                    {isMedia(secondImage) && (
                        <Image src={awsUrl + secondImage.filename} alt={heading} width={600} height={800} className=' translate-y-5 rounded-md transform transition-transform duration-300 hover:scale-105 border-4 border-white' />
                    )}
                </div>
            </div>
